...
function slugFrom(u){if(!u)return '';var s=String(u);var q=s.indexOf('?');if(q>=0)s=s.substring(0,q);var h2=s.indexOf('#');if(h2>=0)s=s.substring(0,h2);var parts=s.split('/');var last='';for(var i=parts.length-1;i>=0;i--){if(parts[i]){last=parts[i];break;}}if(!last)return '';var lo=last.toLowerCase();if(lo.substring(lo.length-5)==='.html')last=last.substring(0,last.length-5);else if(lo.substring(lo.length-4)==='.htm')last=last.substring(0,last.length-4);try{last=decodeURIComponent(last);}catch(e){}return last;}
function artTitle(u){var last=slugFrom(u);if(!last)return '(unknown)';last=last.split('-').join(' ').split('_').join(' ');var out='',up=true;for(var i=0;i<last.length;i++){var c=last.charAt(i);if(c===' '){up=true;out+=c;}else if(up){out+=c.toUpperCase();up=false;}else{out+=c;}}return out.trim()||String(u);}
// Article URL: use backend URL as-is (already full URL like /YYYY/MM/slug.html)
function artUrl(u){if(!u)return SITE+'/';var s=String(u).trim();if(s.substring(0,7)==='http://'||s.substring(0,8)==='https://')return s;while(s.charAt(0)==='/')s=s.substring(1);return SITE+'/'+s;}
// Tool URL: HUB URL + #tool=<slug>
...
function loadOverview(){
  ...
  apiCall(wr('/stats')).then(function(s){
    ...
    var cards=[
      ...
      ['total_tool_uses','Tool Uses','fa-wrench','tools'],
      ['total_comments','Comments','fa-comments','comments'],
      ['total_tool_likes','Tool Likes','fa-thumbs-up','tools'],
      ['total_article_likes','Article Likes','fa-heart','articles']
    ];
    var h='';
    for(var i=0;i<cards.length;i++){var c=cards[i];var v=Number(st[c[0]]||0);h+='<div class="sc" data-pg="'+c[3]+'" style="'+(c[3]?'cursor:pointer':'')+'"><div class="val">'+fmt(v)+'</div><div class="lbl2">'+c[1]+'</div><i class="fa-solid '+c[2]+' ic"></i></div>';}
    G('STG').innerHTML=h;
  ...
  // Daily activity + comments overlay
  ...
  apiCall(wr('/popular-articles?limit=1000')).then(function(d){
    ...
    for(var i=0;i<arts.length;i++){var a=arts[i];var raw=a.url||a.name||a.article_id||'';var t=artTitle(raw);var u=artUrl(raw);var c=Number(a.count||0);var p=Math.round(c/mx*100);h+='<div class="br"><span class="rnk">'+(i+1)+'</span><a class="bn" href="'+esc(u)+'" target="_blank" rel="noopener" title="'+esc(u)+'">'+esc(t)+'</a><span class="bt"><span class="bf" style="width:'+p+'%"></span></span><span class="bv">'+fmt(c)+'</span></div>';}
    G('AVL').innerHTML=h;
